...
class App {
	...
	defaultHandle(request, h) {
		return h
			.response({ statusCode: 401, error: 'Not Implemented', message: 'This feature has not been implemented.' })
			.code(501);
	}
	...
	async handle(operation, request, h) {
		...
		try {
			...
			if (result instanceof Error) {
				return h
					.response({ statusCode: result.code || 500, error: 'Server Error', message: result.message })
			...
			if (process.env.ENV_TYPE === 'development') {
				return h.response({ statusCode: 500, error: 'Server Error', message: error.message }).code(500);
			}
			return h.response({ statusCode: 500, error: 'Server Error', message: error.message }).code(500);
		}
	}
